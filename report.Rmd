...
output:
  html_document:
    css: C:/Users/abdus/AppData/Local/R/win-library/4.3/rpc/app/www/style.css
---


```{r echo=FALSE}
d <- rpc:::score.numeric(responses)
```
...
```{r echo=FALSE}
rpc:::interpret(d, "Agreeableness")
```


The second factor2 is labelled as **Conscientiousness**. Individuals who score high on this factor are careful and diligent. Low scorers are impulsive and disorganized.

```{r echo=FALSE}
rpc:::interpret(d, "Conscientiousness")
```
...
```{r echo=FALSE}
rpc:::interpret(d, "Emotional Stability")
```
...
```{r echo=FALSE}
rpc:::interpret(d, "Intellect/Imagination")
```
